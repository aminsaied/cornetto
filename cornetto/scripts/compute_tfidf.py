"""
This script computes TF-IDF scores for a given vocabulary, 
and allows to reduce the size of it by dropping the words 
with the lowest TF-IDF score. 

It uses a pre-processed arxiv dataset to learn 
the TF-IDF scores, where each text (abstract) 
corresponds to only one label. 
