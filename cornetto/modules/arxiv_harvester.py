...
class ArxivHarvester(object):
    """
    This class is designed to collect metadata on math papers from arxiv.org.
    """
    ...
    def harvest(self, start_date=None, end_date=None):
        """
        *NOTE*: this method takes a while to run due to necessary time 
        gaps between succesive server requests.
        
        Collects the data from arxiv.org into pandas dataframes 
        and saves the dataframes into the desired folder. 
        By default the files are saved into "../data/arxiv/".
        Each dataframe contains 1000 records (this is how much 
        arxiv.org returns at a time.)
        Input:
          -- start_date : str, format 'YYYY-MM-DD'. 
          -- end_date : str, format 'YYYY-MM-DD', defaults to 
          today's date.
    ...
    def _make_request(url):
        ...
        In case of HTTP Error, sleeps 600 seconds 
        (this is how much arxiv.org wants). If the error repeats 
        more than 3 times, it stops trying and returns.
        Input:
          -- url : str
        """
        counter = 0
